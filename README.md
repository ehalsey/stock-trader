...
```bash
state-machine$ sam logs -n StockCheckerFunction --stack-name state-machine --tail
```
...
```bash
state-machine$ cd functions/stock-checker
...
```bash
aws cloudformation delete-stack --stack-name state-machine
```
